				...
				<div class="login_input_cont">
					<div class="test">
						<input type="text" id="loginId" name="loginId" value=""
							placeholder="아이디"> <input type="password" id="loginPwd"
							name="loginPwd" value="" placeholder="비밀번호">
					</div>
					<button type="submit" class="btn_member_login">로그인</button>
					<button type="button" id="btn_member_join" class="btn_member_join" onclick="location.href='<%= request.getContextPath() %>/member/joinAgreement';">회원가입</button>
				</div>
				
				<div class="id_chk">
					<span class="form_element"> <input type="checkbox" id="saveId" name="saveId" value="y" checked="true" /> 
						<label for="saveId" class="on">아이디 저장</label>
					</span>
				</div>
				<ul class="idpw_list">
					<li><button type="button" id="btnFindId" class="btnFindId" onclick="location.href='<%= request.getContextPath() %>/member/findId';">아이디 찾기</button></li>
