...
%>
		<tr>
			<th class="cart_col"><input type="checkbox" name="checkAll" id="checkAll" checked><label for="checkAll"></label></th>
			<th>상품</th>
			<th>수량</th>
			<th>첫배송일</th>
			<th>가격</th>
...
<%
			for (int i = 0; i < cartList.size(); i++) {
				...
				String firstShippingDate = cart.getFirstShipppingDate() != null ?  cart.getFirstShipppingDate() : new Methods().getDefaultFilstShippingDate();
%>
		<tr id="<%= cart.getCartNo()%>" class="itemRow">
			<td class="col1"><input type="checkbox" name="check" id="check<%=cart.getCartNo()%>" checked><label for="check<%=cart.getCartNo()%>"></label></td>
			<td>
				<ul>
					<li>
						<img src="<%= request.getContextPath() %>/upload/product/<%= attach.getRenamedFileName()%>" alt="<%= attach.getOriginalFileName() %>" class="itemImg"></li>
					<li>
			...
			<td class="quantity">
				<table class="quantity_table">
					<tr>
						<td>
							<span class="quantity_minus"><i class="fa-solid fa-minus"></i>
							</span>
						</td>
						<td>
							<input type="number" class="qty" value="<%=cart.getQuantity()%>" readonly><input type="hidden" value="<%= product.getProductPrice() %>" />
						</td>
						<td>
							<span class="quantity_plus"><i class="fa-solid fa-plus"></i></span></td>
					</tr>
				</table>
			</td>
			<td>
				<input class="datepicker" value="<%= firstShippingDate %>">
			</td>
			<td>
				<span class="price"><%= product.getProductPrice()*cart.getQuantity()%></span>원
			</td>
		</tr>
<%
        	}
